---
editUrl: false
next: false
prev: false
title: "@kodkord/classes"
---

## Classes

- [Entity](/api/classes/classes/entity/)
- [Channel](/api/classes/classes/channel/)
- [Emoji](/api/classes/classes/emoji/)
- [Guild](/api/classes/classes/guild/)
- [HashImage](/api/classes/classes/hashimage/)
- [UserAvatar](/api/classes/classes/useravatar/)
- [UserBanner](/api/classes/classes/userbanner/)
- [GuildIcon](/api/classes/classes/guildicon/)
- [GuildBanner](/api/classes/classes/guildbanner/)
- [RoleIcon](/api/classes/classes/roleicon/)
- [MemberAvatar](/api/classes/classes/memberavatar/)
- [MemberBanner](/api/classes/classes/memberbanner/)
- [Interaction](/api/classes/classes/interaction/)
- [Member](/api/classes/classes/member/)
- [Message](/api/classes/classes/message/)
- [Role](/api/classes/classes/role/)
- [Sticker](/api/classes/classes/sticker/)
- [User](/api/classes/classes/user/)

## Interfaces

- [RawImage](/api/classes/interfaces/rawimage/)

## Type Aliases

- [Sizes](/api/classes/type-aliases/sizes/)
- [GuildRawImage](/api/classes/type-aliases/guildrawimage/)
- [MessageData](/api/classes/type-aliases/messagedata/)
